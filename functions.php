...
function BRAVOSP_Translate($html) {
  //no mystery here, we take the translations from database
    $sql="SELECT * FROM `wp_bravo_sp`  ORDER BY CHAR_LENGTH(`searchFor`) DESC";
    global $wpdb;
    ...
    foreach($results as $clave=>$tr){
        ...
        $html=BRAVOSP_Analyse_HTML($tr->searchFor,$tr->replaceBy,$html);
...
function BRAVOSP_Analyse_HTML($searchPattern,$replace,$html){
              ...
              for($e=0;$char!="<";$e++){
                  ...
                  if($char=="<"){
                     ...
                     if(strpos(" ".$cadeneta,"/")!=false) {
                      ...
                      $insideWord=BRAVOSP_insideWord($prevChar,$nextChar);
                      //we retrive the allowed tags
                      $tags=BRAVOSP_allowedTagsBetween();
...
function BRAVOSP_insideWord($prevChar,$nextChar){
        $prev=false;
        $next=false;
...
function BRAVOSP_allowedTagsBetween(){
...
add_action('wp_loaded', 'BRAVOSP_start');
add_action('shutdown', 'BRAVOSP_end');


function BRAVOSP_begin_ob(){
  error_log("sii");
  global $_BRAVOSP_OB;
  $_BRAVOSP_OB=true;
  ob_start("BRAVOSP_Translate"); 
}


function BRAVOSP_start() {
  ...
  if(strpos(" ".$uri,"/wp-admin/")!=false) {
    ...
    error_log("ha entrado".$uri);
    BRAVOSP_begin_ob();
  }
}


function BRAVOSP_end() {
